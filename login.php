...
{	
	echo "you have successfully registered. please login for security reason";
}
...
$password=md5($_POST["password"]);
	if(!empty($_POST["username"]) && !empty($_POST["password"]))
		{
			$query="select id,username,password,relation FROM loginpage WHERE username='".$username."' AND password='".$password."'";
			if($string=mysql_query($query))
			{
					$num=mysql_num_rows($string);
			if($num==1){
				echo "login successful";
				$id=mysql_result($string,0,'id');
				$_SESSION['userid']=$id;
				$d=mysql_result($string,0,'relation');
				$_SESSION['doctor']=0;
				if($d == "doctor")
					{$_SESSION['doctor']=1;}
